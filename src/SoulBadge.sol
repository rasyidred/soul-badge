...
contract SoulBadge is ERC721URIStorage, Ownable, ReentrancyGuard, ERC721Burnable {
    ...
    modifier checkValid(bytes32 nullifier) {
        require(eventDetails[nullifier].isExist == true, "The code is not valid nor exist");
        _;
    }
    ...
    event eventCreated(uint256 maxAttendees, string eventInfo, bytes32 nullifier);
    ...
    function mintNFTs(uint256 _noOfAttendees, string memory _tokenURI)
        public
        onlyOwner
        nonReentrant
        returns (bytes32 nullifier)
    {
        nullifier = keccak256(abi.encodePacked(block.timestamp, _noOfAttendees, _tokenURI));
        uint256 startingTokenId;
        ...
        uint256 endingTokenId = startingTokenId + _noOfAttendees;
        ...
        for (uint256 i = 0; i < _noOfAttendees; i++) {
    ...
    function claim(bytes32 _nullifier, address _to)
        public
        checkValid(_nullifier)
        nonReentrant
        returns (uint256 tokenId, uint256 remainingTokens)
    {
        ...
        eventDetails[_nullifier].attendeeDetails[_to] = Claimer(_to, tokenId, true);
    ...
    /* Getter Functions */
    function getRemainingTokens(bytes32 nullifier) internal view returns (uint256 remainingTokens) {
        uint256 startingTokenId = eventDetails[nullifier].startingTokenId;
        uint256 endingTokenId = eventDetails[nullifier].endingTokenId;
        uint256 claimed = eventDetails[nullifier].counterClaimed;
        remainingTokens = endingTokenId - startingTokenId - claimed;
    }

    function getTokenId(bytes32 nullifier) internal view returns (uint256 tokenId) {
        tokenId = eventDetails[nullifier].counterClaimed + eventDetails[nullifier].startingTokenId;
    }

    function getClaimer(bytes32 nullifier)
        public
        view
        returns (address[] memory addresses, uint256[] memory tokenIds, bool[] memory claimed)
    {
        addresses = new address[](eventDetails[nullifier].maxAttendees);
        tokenIds = new uint256[](eventDetails[nullifier].maxAttendees);
        claimed = new bool[](eventDetails[nullifier].maxAttendees);

        for (uint256 i; i < eventDetails[nullifier].maxAttendees; i++) {
    ...
    function _update(address to, uint256 tokenId, address auth) internal override returns (address) {
        ...
        require(
            from == address(0) || to == address(0) || from == storageAddress || to == storageAddress,
...
contract BadgeStorage is ReentrancyGuard, Ownable {
    ...
    event ItemStored(address indexed nftContract, uint256 indexed tokenId, address creator, address owner, bool claim);

    event badgeClaimed(address recipient, uint256 tokenId, uint256 remainingTokens);

    /* Stores an NFT on the storage before distributed */
    // create market item
    function storeItem(address nftContract, uint256 tokenId) public payable nonReentrant onlyOwner {
    ...
    // buy nft
    function claimBadge(address nftContract, bytes32 nullifier) public nonReentrant {
        SoulBadge _SoulBadge = SoulBadge(nftContract);

        require(msg.sender != owner(), "Only attendees that can claim!");
        (uint256 tokenId, uint256 remainingTokens) = _SoulBadge.claim(nullifier, msg.sender);
